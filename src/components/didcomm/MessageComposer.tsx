...
export function MessageComposer({
  ...
  content = '',
  threadId,
  messageType = MESSAGE_TYPES.BASIC_MESSAGE,
  showAdvanced = true,
  onMessageSent,
  onCancel,
  className
  ...
  return (
    <Card className={cn('w-full', className)}>
      <CardHeader>
        ...
        <CardDescription>
          Create and send DIDComm messages with attachments and various protocols
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        {showAdvanced && (
          <div className="space-y-2">
            ...
            <Select
              value={formData.type}
              onValueChange={(value: MessageType) => setFormData(prev => ({ ...prev, type: value }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
        ...
        <div className="flex gap-3">
          <Button
            onClick={sendMessage}
            disabled={loading || formData.to.length === 0}
            className="flex-1"
          >
            {loading ? (
              <>
