...
export const ConsentForm: React.FC<ConsentFormProps> = ({
  ...
  const hasSensitiveScopes = consentDetails.requestedScopes.some(
    scope => getScopeInfo(scope).sensitive
  )
      ...
      <CardHeader className="text-center">
        <div className="flex justify-center mb-4">
          {consentDetails.clientLogoUri ? (
            <Image
              src={consentDetails.clientLogoUri}
              alt={`${consentDetails.clientName} logo`}
              width={64}
              height={64}
            ...
            <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center">
              <span className="text-white text-2xl font-bold">
                {consentDetails.clientName.charAt(0).toUpperCase()}
              </span>
            </div>
          )}
        </div>
        ...
        <CardTitle className="text-xl font-semibold">
          Grant Access to {consentDetails.clientName}
        </CardTitle>
        
        <p className="text-sm text-muted-foreground mt-2">
          This application is requesting access to your account. Review the permissions below.
        </p>
      </CardHeader>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium text-amber-800">
                This app is requesting sensitive information
              </span>
            </div>
            <p className="text-xs text-amber-700 mt-1">
              Please review all permissions carefully before granting access.
            </p>
          </div>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2">
                      <span className="font-medium text-sm">{scopeInfo.scope}</span>
                      {scopeInfo.sensitive && (
