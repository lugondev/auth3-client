...
import {Trash2, Download, Shield, XCircle, CheckCircle, AlertTriangle, FileDown, Package, Clock} from 'lucide-react'
 ...
 */
export function BulkCredentialManager({
	credentials,
	onCredentialsUpdated,
	className = ''
}: BulkCredentialManagerProps) {
	...
	const handleBulkDelete = async () => {
		...
		const confirmed = window.confirm(
			`Are you sure you want to delete ${selectedCredentials.size} credential(s)? This action cannot be undone.`
		)
		...
		<div className={className}>
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Select multiple credentials to perform bulk operations
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					...
					<div className="flex items-center justify-between p-3 bg-muted rounded-md">
						...
						{selectedCount > 0 && (
							<div className="text-sm text-muted-foreground">
								{activeSelectedCount} active credential(s) eligible for revocation
							</div>
						)}
					</div>
					...
					<div className="flex gap-2 flex-wrap">
						<Button
							variant="outline"
							size="sm"
							onClick={() => setSelectedCredentials(new Set(getSelectableCredentials('active').map(c => c.id)))}
						>
						...
						<Button
							variant="outline"
							size="sm"
							onClick={() => setSelectedCredentials(new Set(getSelectableCredentials('revoked').map(c => c.id)))}
						>
						...
						<Button
							variant="outline"
							size="sm"
							onClick={() => setSelectedCredentials(new Set(getSelectableCredentials('expired').map(c => c.id)))}
						>
					...
					{selectedCount > activeSelectedCount && (
						<Alert>
							...
							<AlertDescription>
								{selectedCount - activeSelectedCount} selected credential(s) are not active and cannot be revoked.
							</AlertDescription>
						</Alert>
					)}
						...
						{credentials.map(credential => (
							<div
								...
								className={`flex items-center gap-3 p-3 border rounded-md ${
									selectedCredentials.has(credential.id) ? 'bg-blue-50 border-blue-200' : ''
								}`}
							>
								<Checkbox
			...
			<Dialog open={showProgressModal} onOpenChange={setShowProgressModal}>
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Processing {operations.length} credential(s)...
						</DialogDescription>
					</DialogHeader>
					
					<div className="space-y-4">
						<Progress 
							value={(operations.filter(op => op.status === 'completed' || op.status === 'failed').length / operations.length) * 100} 
						/>
						...
						<div className="space-y-2 max-h-64 overflow-y-auto">
							{operations.map(operation => (
								<div key={operation.id} className="flex items-center gap-2 text-sm">
									...
									{operation.error && (
										<span className="text-xs text-red-600">{operation.error}</span>
									)}
								</div>
							))}
						</div>
