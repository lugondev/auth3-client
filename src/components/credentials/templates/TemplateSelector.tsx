...
export function TemplateSelector({selectedTemplate, onTemplateSelect, onCreateCustom, className = '', showAnalytics = false, enableBulkSelection = false, selectedTemplates = [], onBulkSelectionChange, viewMode: initialViewMode = 'grid', compactMode = false}: TemplateSelectorProps) {
	...
	const handleSelectAll = () => {
		...
		const allSelected = filteredTemplates.every((t) => selectedTemplates.some((st) => st.id === t.id))

		if (allSelected) {
			// Deselect all filtered templates
			const newSelection = selectedTemplates.filter((st) => !filteredTemplates.some((ft) => ft.id === st.id))
			onBulkSelectionChange?.(newSelection)
		} else {
			// Select all filtered templates
			const templatesToAdd = filteredTemplates.filter((ft) => !selectedTemplates.some((st) => st.id === ft.id))
	...
	const renderTemplateCard = (template: EnhancedTemplate) => {
		const isSelected = enableBulkSelection ? selectedTemplates.some((t) => t.id === template.id) : selectedTemplate?.id === template.id

		const cardClasses = `cursor-pointer transition-all hover:shadow-md ${isSelected ? 'ring-2 ring-primary bg-primary/5' : ''} ${compactMode ? 'p-3' : 'p-4'}`

		if (viewMode === 'list') {
			return (
				<Card key={template.id} className={cardClasses} onClick={() => handleTemplateSelect(template)}>
					<CardContent className={compactMode ? 'p-3' : 'p-4'}>
						<div className='flex items-center gap-4'>
							{/* Icon */}
							<div className='flex-shrink-0'>{getTemplateIcon(template)}</div>

							{/* Content */}
							<div className='flex-1 min-w-0'>
								<div className='flex items-center justify-between gap-2'>
									<div className='flex-1'>
										<h4 className={`font-semibold ${compactMode ? 'text-sm' : 'text-base'} line-clamp-1`}>{template.name}</h4>
										<p className={`text-muted-foreground ${compactMode ? 'text-xs' : 'text-sm'} line-clamp-1 mt-1`}>{template.description}</p>
									</div>
			...
			<Card key={template.id} className={cardClasses} onClick={() => handleTemplateSelect(template)}>
				<CardContent className={compactMode ? 'p-3' : 'p-4'}>
					<div className='flex items-start gap-3'>
						{/* Icon */}
						<div className='flex-shrink-0 mt-1'>{getTemplateIcon(template)}</div>
						...
						<div className='flex-1 min-w-0'>
							<div className='flex items-start justify-between gap-2'>
								<div className='flex-1'>
									<h4 className={`font-semibold ${compactMode ? 'text-sm' : 'text-base'} line-clamp-1`}>{template.name}</h4>
									<p className={`text-muted-foreground ${compactMode ? 'text-xs' : 'text-sm'} line-clamp-2 mt-1`}>{template.description}</p>
								</div>
							...
							{!compactMode && template.schema && typeof template.schema === 'object' && (
								<div className='mt-3'>
									<Label className='text-xs text-muted-foreground'>Fields ({Object.keys(template.schema).length})</Label>
	...
	return (
		<Card className={className}>
			<CardHeader className={compactMode ? 'pb-4' : 'pb-6'}>
				...
				{!compactMode && <CardDescription>{enableBulkSelection ? 'Select multiple templates for bulk operations' : 'Choose a template for credential issuance or create a custom one'}</CardDescription>}
			</CardHeader>
					...
					<div className='flex gap-2 flex-wrap'>
						...
						{enableBulkSelection && filteredTemplates.length > 0 && (
							<Button variant='outline' size='sm' onClick={handleSelectAll}>
								{filteredTemplates.every((t) => selectedTemplates.some((st) => st.id === t.id)) ? 'Deselect All' : 'Select All'}
							</Button>
						)}
					</div>
				</div>
				...
				<ScrollArea className={compactMode ? 'h-[300px] w-full' : 'h-[500px] w-full'}>
					{filteredTemplates.length === 0 ? (
						<div className='text-center py-12'>
							...
							<p className='text-muted-foreground mb-4'>{searchTerm ? 'No templates match your current search.' : 'No templates are available.'}</p>
							{onCreateCustom && (
						...
						<div className='space-y-4'>
							...
							{onCreateCustom && !enableBulkSelection && (
								<Card className={`cursor-pointer transition-all hover:shadow-md border-dashed ${!selectedTemplate ? 'ring-2 ring-primary bg-primary/5' : 'hover:border-primary/50'}`} onClick={() => onTemplateSelect(null)}>
									<CardContent className={compactMode ? 'p-3' : 'p-4'}>
										<div className='flex items-center gap-3'>
											...
											<div className='flex-1'>
												<h4 className={`font-semibold ${compactMode ? 'text-sm' : 'text-base'}`}>Custom Credential</h4>
												<p className={`text-muted-foreground ${compactMode ? 'text-xs' : 'text-sm'} mt-1`}>Create a credential with custom fields and structure</p>
											</div>
							...
							{/* Templates Grid/List */}
							<div className={`${viewMode === 'grid' ? `grid ${compactMode ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'} gap-3` : 'space-y-2'}`}>{filteredTemplates.map(renderTemplateCard)}</div>
						</div>
					)}
				</ScrollArea>
										...
										<div className='grid grid-cols-2 md:grid-cols-3 gap-2 mt-2'>
											{Object.entries(selectedTemplate.schema).map(([fieldName, field]) => (
												<div key={fieldName} className='text-sm'>
													...
													<span className='text-muted-foreground ml-1'>({typeof field === 'object' && field !== null ? 'object' : typeof field})</span>
												</div>
											))}
										</div>
									</div>
