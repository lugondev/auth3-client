...
import {Activity, Wifi, WifiOff, RefreshCw, Bell, AlertTriangle, CheckCircle, Clock} from 'lucide-react'
 ...
 */
export function RealTimeStatusMonitor({credentials, onStatusChange, className = ''}: RealTimeStatusMonitorProps) {
	...
	const simulateStatusUpdate = useCallback(() => {
		...
		if (Math.random() < 0.1) {
			...
			const possibleStatuses: CredentialStatus[] = [CredentialStatus.ACTIVE, CredentialStatus.REVOKED, CredentialStatus.SUSPENDED, CredentialStatus.EXPIRED]
			...
			const update: StatusUpdate = {
				...
				credentialTitle: Array.isArray(credential.type) ? credential.type.filter((t) => t !== 'VerifiableCredential').join(', ') : credential.type,
	...
	const handleManualRefresh = async () => {
		try {
			...
			for (const credential of credentials.slice(0, 5)) {
				// Check first 5 to avoid API overload
				try {
					const statusResponse = await getCredentialStatus(credential.id)

					// Map RevocationStatusResponse to CredentialStatus
					const currentStatus = statusResponse.revoked ? CredentialStatus.REVOKED : CredentialStatus.ACTIVE
				...
				<CardHeader>
					<div className='flex items-center justify-between'>
						<div>
							...
							<CardDescription>Monitor credential status changes in real-time</CardDescription>
						</div>

						<div className='flex items-center gap-2'>
							{/* Connection Status */}
							<div className='flex items-center gap-2'>
								{isConnected ? <Wifi className='h-4 w-4 text-green-600' /> : <WifiOff className='h-4 w-4 text-gray-400' />}
							...
							<div className='flex items-center gap-2'>
								...
								<Switch checked={isMonitoring} onCheckedChange={toggleMonitoring} disabled={credentials.length === 0} />
							</div>
						</div>
					</div>
				</CardHeader>
				...
				<CardContent className='space-y-4'>
					...
					<div className='flex items-center justify-between p-3 bg-muted rounded-md'>
						<div className='flex items-center gap-4'>
							<div>
								...
								<div className='text-xs text-muted-foreground'>{credentials.length} credential(s) being monitored</div>
							</div>

							{lastUpdateTime && (
								<div>
									<div className='text-sm font-medium'>Last Update</div>
									<div className='text-xs text-muted-foreground'>{formatTimestamp(lastUpdateTime)}</div>
								</div>
							)}
						</div>
					...
					<div className='space-y-2'>
						<div className='flex items-center justify-between'>
							...
							<Badge variant='outline'>{statusUpdates.length} update(s)</Badge>
						</div>
						...
						{statusUpdates.length === 0 ? (
							<div className='text-center py-8'>
								...
								<p className='text-sm text-muted-foreground mt-1'>{isMonitoring ? 'Monitoring active...' : 'Enable monitoring to track changes'}</p>
							</div>
						) : (
											...
											<div className='flex items-start gap-3 p-3 rounded-md border'>
												...
												<div className='flex-1 min-w-0'>
													<div className='flex items-center gap-2 mb-1'>
														<span className='font-medium text-sm truncate'>{update.credentialTitle}</span>
													...
													<div className='flex items-center gap-2 text-sm'>
														<Badge variant={getStatusBadgeVariant(update.previousStatus)}>{update.previousStatus}</Badge>
														<span>→</span>
														<Badge variant={getStatusBadgeVariant(update.newStatus)}>{update.newStatus}</Badge>
													</div>

													{update.reason && <p className='text-xs text-muted-foreground mt-1'>{update.reason}</p>}
					...
					{statusUpdates.length > 0 && (
						<div className='grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t'>
							<div className='text-center'>
								<div className='text-lg font-bold text-green-600'>{statusUpdates.filter((u) => u.newStatus === CredentialStatus.ACTIVE).length}</div>
								<div className='text-xs text-muted-foreground'>Activated</div>
							</div>
							<div className='text-center'>
								<div className='text-lg font-bold text-red-600'>{statusUpdates.filter((u) => u.newStatus === CredentialStatus.REVOKED).length}</div>
								<div className='text-xs text-muted-foreground'>Revoked</div>
							</div>
							<div className='text-center'>
								<div className='text-lg font-bold text-yellow-600'>{statusUpdates.filter((u) => u.newStatus === CredentialStatus.SUSPENDED).length}</div>
								<div className='text-xs text-muted-foreground'>Suspended</div>
							</div>
							<div className='text-center'>
								<div className='text-lg font-bold text-gray-600'>{statusUpdates.filter((u) => u.newStatus === CredentialStatus.EXPIRED).length}</div>
