		...
		return (
			<Card className={className}>
				...
				<CardContent>
					...
					<Button variant='outline' size='sm' className='mt-2' onClick={loadTransitions}>
						Retry
					</Button>
				</CardContent>
			</Card>
				...
				<div className='space-y-4'>
					{transitions.map((transition, index) => (
						<div key={transition.id} className='relative'>
							{/* Timeline line */}
							{index < transitions.length - 1 && <div className='absolute left-4 top-8 bottom-0 w-px bg-gray-200' />}

							<div className='flex items-start gap-4'>
								{/* Status indicator */}
								<div className='relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white border-2 border-gray-200'>{getStateIcon(transition)}</div>
									...
									<div className='flex items-start justify-between'>
										<div className='flex-1'>
											<div className='flex items-center gap-2 mb-1'>
												<Badge variant='outline' className={getStateBadgeColor(transition.fromState)}>
													{transition.fromState || 'unknown'}
												</Badge>
												<span className='text-gray-400'>→</span>
												<Badge variant='outline' className={getStateBadgeColor(transition.toState)}>
											...
											<div className='flex items-center gap-4 text-sm text-gray-600'>
												...
												<span title={formatTimestampTitle(transition.timestamp)}>{formatTimestamp(transition.timestamp)}</span>
											</div>

											{transition.errorMessage && <div className='mt-2 p-2 bg-red-50 border border-red-200 rounded text-sm text-red-700'>{transition.errorMessage}</div>}
										</div>
										...
										{showMetadata && transition.metadata && (
											<Collapsible>
												<CollapsibleTrigger asChild>
													<Button variant='ghost' size='sm' className='h-6 w-6 p-0' onClick={() => toggleExpanded(transition.id)}>
														{expandedItems.has(transition.id) ? <ChevronDown className='h-3 w-3' /> : <ChevronRight className='h-3 w-3' />}
													</Button>
												</CollapsibleTrigger>
												<CollapsibleContent>
													<div className='mt-2 p-3 bg-gray-50 rounded text-xs'>
														<pre className='whitespace-pre-wrap text-gray-700'>{JSON.stringify(transition.metadata, null, 2)}</pre>
													</div>
												</CollapsibleContent>
											</Collapsible>
										)}
				...
				{autoRefresh && (
					<div className='mt-4 pt-4 border-t border-gray-200'>
						<div className='flex items-center justify-between text-xs text-gray-500'>
							...
							<Button variant='outline' size='sm' onClick={loadTransitions} className='h-6 px-2 py-1 text-xs'>
								Refresh Now
							</Button>
						</div>
					</div>
