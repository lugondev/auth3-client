...
import {History, Search, Download, Eye, CheckCircle, XCircle, Clock, Calendar, User, FileText, RefreshCw, AlertCircle} from 'lucide-react'
import {toast} from 'sonner'
import {historyService, type HistoryItem, type HistoryFilters} from '@/services/historyService'
...
export const HistoryView: React.FC = () => {
	...
	const handleExport = () => {
		...
		// Simple CSV export
		const csvData = [['ID', 'Type', 'Title', 'Status', 'Timestamp', 'Verifier', 'Holder', 'Credential Types'], ...historyItems.map((item) => [item.id, item.type, item.title, item.status, item.timestamp, item.verifier, item.holder || '', item.details.credentialTypes?.join('; ') || ''])]
					...
					<div className='flex flex-col sm:flex-row gap-4'>
						<div className='flex-1'>
							<div className='relative'>
								...
								<Input placeholder='Search by title, description, or verifier...' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className='pl-10' />
							</div>
						</div>
						<div className='flex gap-2'>
								...
								<CardContent className='p-6'>
									<div className='flex items-start justify-between'>
										<div className='flex items-start gap-4 flex-1'>
											{/* Type Icon */}
											<div className='p-2 bg-gray-100 rounded-lg'>{getTypeIcon(item.type)}</div>
													...
													{item.details.responseCount !== undefined && (
														<div>
															...
															<p className='font-medium'>
																{item.details.responseCount} / {item.details.responseTotal || 'N/A'}
															</p>
														</div>
													)}
													...
													<div className='mt-3'>
														...
														<div className='flex flex-wrap gap-2'>
															{item.details.allResponses.slice(0, 3).map((response, index) => (
																<Badge key={response.id} variant='outline' className={`text-xs ${response.status === 'verified' ? 'border-green-300 text-green-700' : 'border-yellow-300 text-yellow-700'}`}>
										...
										<div className='flex flex-col items-end gap-2'>
											...
											<div className='flex gap-1 flex-col'>
												<Button variant='ghost' size='sm' onClick={() => handleViewDetails(item)} title='View request details'>
													<Eye className='h-4 w-4 mr-1' />
													Request
												</Button>
												{item.details.responseCount && item.details.responseCount > 0 && (
													<Button variant='ghost' size='sm' onClick={() => handleViewResponses(item)} title={`View all ${item.details.responseCount} responses`}>
														<User className='h-4 w-4 mr-1' />
														Responses ({item.details.responseCount})
													</Button>
												)}
												{item.details.presentationId && (
													<Button variant='ghost' size='sm' onClick={() => handleViewPresentation(item)} title='View latest presentation details'>
