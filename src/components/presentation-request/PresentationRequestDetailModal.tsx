...
import {Copy, QrCode, Calendar, Clock, Shield, Users, Eye, ExternalLink, CheckCircle, XCircle, AlertCircle, Settings} from 'lucide-react'
...
export function PresentationRequestDetailModal({request, isOpen, onClose, onEdit}: PresentationRequestDetailModalProps) {
		...
		<Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
			<DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
				<DialogHeader>
					...
					<DialogDescription>Presentation Request Details - ID: {request.request_id}</DialogDescription>
				</DialogHeader>

				{/* Tabs */}
				<div className='flex border-b'>
					<button className={`px-4 py-2 text-sm font-medium ${activeTab === 'details' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setActiveTab('details')}>
						Details
					</button>
					<button className={`px-4 py-2 text-sm font-medium ${activeTab === 'responses' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setActiveTab('responses')}>
						Responses ({responses.length})
					</button>
					<button className={`px-4 py-2 text-sm font-medium ${activeTab === 'analytics' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setActiveTab('analytics')}>
						Analytics
					</button>
				</div>
									...
									<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
										...
										<div>
											...
											<div className='mt-1 flex items-center gap-2'>
												...
												{request.max_responses && <span className='text-muted-foreground'>/ {request.max_responses}</span>}
											</div>
										</div>
									</div>
							...
							<Card>
								...
								<CardContent className='space-y-4'>
									...
									<div>
										...
										<div className='mt-1 flex items-center gap-2'>
											<code className='text-xs bg-muted p-2 rounded font-mono flex-1 break-all'>{request.verifier_did}</code>
											<Button variant='outline' size='sm' onClick={() => copyToClipboard(request.verifier_did, 'Verifier DID')}>
											...
											<div key={index} className='border rounded-lg p-4'>
												<div className='flex items-center justify-between mb-2'>
													...
													<div className='flex items-center gap-2'>
														<Badge variant={credential.essential ? 'default' : 'secondary'}>{credential.essential ? 'Essential' : 'Optional'}</Badge>
														{credential.format && <Badge variant='outline'>{credential.format}</Badge>}
													</div>
												</div>

												{credential.purpose && <p className='text-sm text-muted-foreground mb-2'>{credential.purpose}</p>}
												...
												{credential.constraints && Object.keys(credential.constraints).length > 0 && (
													<div className='mt-2'>
														...
														<pre className='pre-code-sm'>{JSON.stringify(credential.constraints, null, 2)}</pre>
													</div>
												)}
											</div>
										))}
									...
									<div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
										{Object.entries(request.verification_options || {}).map(([option, enabled]) => (
											<div key={option} className='flex items-center gap-2'>
												{enabled ? <CheckCircle className='h-4 w-4 text-green-600' /> : <XCircle className='h-4 w-4 text-gray-400' />}
												<span className='text-sm'>{option.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())}</span>
											</div>
										))}
									</div>
								</CardContent>
							...
							<Card>
								...
								<CardContent className='space-y-4'>
									<div>
										...
										<div className='mt-1 flex items-center gap-2'>
											<code className='text-xs bg-muted p-2 rounded font-mono flex-1'>{request.request_id}</code>
											<Button variant='outline' size='sm' onClick={() => copyToClipboard(request.request_id, 'Request ID')}>
												<Copy className='h-4 w-4' />
											</Button>
										</div>
									</div>
									...
									{request.share_url && (
										<div>
											...
											<div className='mt-1 flex items-center gap-2'>
												<code className='text-xs bg-muted p-2 rounded font-mono flex-1 break-all'>{request.share_url}</code>
												<Button variant='outline' size='sm' onClick={() => copyToClipboard(request.share_url!, 'Share URL')}>
													<Copy className='h-4 w-4' />
												</Button>
												<Button variant='outline' size='sm' onClick={() => window.open(request.share_url, '_blank')}>
									...
									{request.qr_code_data && (
										<div>
											...
											<div className='mt-1 flex items-center gap-2'>
												<code className='text-xs bg-muted p-2 rounded font-mono flex-1 break-all'>QR Code available</code>
												<Button variant='outline' size='sm' onClick={() => copyToClipboard(request.qr_code_data!, 'QR Code Data')}>
													<Copy className='h-4 w-4' />
												</Button>
					...
					{activeTab === 'responses' && (
						<Card>
							<CardHeader>
								...
								<CardDescription>{loadingResponses ? 'Loading responses...' : `${responses.length} response(s) received`}</CardDescription>
							</CardHeader>
							<CardContent>
								{loadingResponses ? (
											...
											<div key={response.id} className='border rounded-lg p-4'>
												<div className='flex items-center justify-between mb-2'>
													<div>
														...
														<p className='text-sm text-muted-foreground'>From: {response.holder_did}</p>
													</div>
													{getStatusBadge(response.status)}
												</div>

												<div className='text-sm text-muted-foreground'>Submitted: {formatDate(response.submitted_at)}</div>

												{response.verified_at && <div className='text-sm text-muted-foreground'>Verified: {formatDate(response.verified_at)}</div>}
											</div>
										))}
									</div>
								)}
						...
						<Card>
							...
							<CardContent>
								<div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
									...
									<div className='text-center p-4 border rounded-lg'>
										<div className='text-2xl font-bold'>{responses.filter((r) => r.status === 'verified').length}</div>
				...
				<div className='flex justify-between gap-4 pt-6 border-t'>
					<div className='flex gap-2'>
						...
						{request.share_url && (
							<Button variant='outline' onClick={() => window.open(request.share_url, '_blank')}>
