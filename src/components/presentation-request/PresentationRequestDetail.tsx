...
import {ArrowLeft, QrCode, Share2, Copy, CheckCircle, XCircle, Clock, MoreHorizontal, Shield, Calendar, Users, Eye, Download} from 'lucide-react'
import {toast} from 'sonner'
import {presentationRequestService} from '@/services/presentation-request-service'
import type {PresentationRequest, PresentationResponse, CredentialRequirement} from '@/types/presentation-request'
...
export function PresentationRequestDetail({requestId, onBack}: PresentationRequestDetailProps) {
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				setLoading(true)
				const requestData = await presentationRequestService.getRequestById(requestId)
				setRequest(requestData)

				setResponsesLoading(true)
				const responseData = await presentationRequestService.getResponses(requestId)
				setResponses(responseData.responses || [])
			} catch {
				console.error('Failed to load data')
				toast.error('Failed to load presentation request data')
			} finally {
				setLoading(false)
							...
							<CardContent className='space-y-4'>
								<div className='grid grid-cols-2 gap-4'>
									...
									<div className='text-center p-4 bg-green-50 rounded-lg'>
										<div className='text-2xl font-bold text-green-600'>{responses.filter((r) => r.status === 'verified').length}</div>
				...
				<TabsContent value='responses'>
					<Card>
						<CardHeader>
							...
							<CardDescription>Responses submitted by credential holders</CardDescription>
						</CardHeader>
						<CardContent>
							{responsesLoading ? (
									...
									<TableBody>
										{responses.map((response) => (
											<TableRow key={response.id}>
												<TableCell>
													<div className='font-mono text-sm break-all max-w-[200px]'>{response.holder_did}</div>
												</TableCell>

												<TableCell>{getStatusBadge(response.status)}</TableCell>

												<TableCell>
													<div className='text-sm'>{formatDate(response.submitted_at)}</div>
												</TableCell>

												<TableCell>{response.verified_at ? <div className='text-sm'>{formatDate(response.verified_at)}</div> : <span className='text-gray-400'>-</span>}</TableCell>
				...
				<TabsContent value='credentials'>
					<Card>
						<CardHeader>
							...
							<CardDescription>Credentials that holders must provide</CardDescription>
						</CardHeader>
						<CardContent>
							<div className='space-y-4'>
								{request.required_credentials.map((credential: CredentialRequirement, index: number) => (
			...
			<Dialog open={showQRDialog} onOpenChange={setShowQRDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>Scan this QR code to access the presentation request</DialogDescription>
					</DialogHeader>
