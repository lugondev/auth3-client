...
import { multiTenantTokenManager, validateTenantTokens } from '@/lib/multi-tenant-token-manager'
...
export const TenantGuard: React.FC<TenantGuardProps> = ({
  ...
  requiredPermissions = [],
  fallbackPath = '/dashboard/tenant-management'
}) => {
  ...
  const withRequestDeduplication = async (key: string, fn: () => Promise<any>) => {
  ...
  const attemptTokenRefresh = async () => {
    return withRequestDeduplication(`refresh-${tenantId}`, async () => {
      try {
        ...
        // Skip validation when refreshing tokens in tenant space to avoid validation errors
        const skipValidation = getCurrentContextMode() === 'tenant'
        const result = await loginTenantContext(tenantId, true, !skipValidation)
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold">
                  {guardState.status === 'loading' ? 'Loading...' : 'Validating Access...'}
                </h3>
                <p className="text-sm text-muted-foreground mt-1">
                  {guardState.status === 'loading' 
                    ? 'Preparing tenant access' 
                    : `Checking access to tenant ${tenantId}`
                  }
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  disabled={guardState.isRefreshing}
                  className="flex-1"
                >
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button 
                  onClick={() => router.push('/dashboard/tenant-management')} 
                  variant="default" 
                  className="flex-1"
                >
                  Tenant Management
