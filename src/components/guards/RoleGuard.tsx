...
export function RoleGuard({children, role, roles, requireAll = false, requireTenant = false, tenantId, allowSystemAdmin = true, loadingComponent, accessDeniedComponent, errorComponent, hideOnDenied = false, redirectOnDenied, silentFail = false, onAccessGranted, onAccessDenied, onError}: RoleGuardProps) {
	const router = useRouter()
	const {isAuthenticated, user, currentTenantId} = useAuth()
	const {hasRole, hasAnyRole, hasAllRoles, isSystemAdmin, loading, error} = usePermissions()
	...
	const checkAccess = React.useCallback((): AccessCheckResult => {
		...
		return {hasAccess: true}
	}, [isAuthenticated, user, currentTenantId, requireTenant, tenantId, allowSystemAdmin, isSystemAdmin, role, roles, requireAll, hasRole, hasAnyRole, hasAllRoles])
...
export function RequireSystemAdmin({children, ...props}: Omit<RoleGuardProps, 'role'>) {
	return (
...
export function RequireTenantAdmin({children, ...props}: Omit<RoleGuardProps, 'role' | 'requireTenant'>) {
	return (
...
export function RequireAdmin({children, ...props}: Omit<RoleGuardProps, 'roles'>) {
	return (
...
export function RequireUser({children, ...props}: Omit<RoleGuardProps, 'roles'>) {
	return (
...
export function RequireAnyRole({roles, children, ...props}: Omit<RoleGuardProps, 'roles' | 'requireAll'> & {roles: string[]}) {
	return (
...
export function RequireAllRoles({roles, children, ...props}: Omit<RoleGuardProps, 'roles' | 'requireAll'> & {roles: string[]}) {
	return (
