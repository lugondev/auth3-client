...
export function PresentationRequestViewer({ 
  request, 
  onSubmit, 
  onBack, 
  isSubmitting 
}: PresentationRequestViewerProps) {
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      ...
      {isExpired && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            This presentation request has expired on {formatDate(request.expires_at!)}
          </AlertDescription>
        </Alert>
      )}
      ...
      {request.status !== 'active' && (
        <Alert>
          ...
          <AlertDescription>
            This request is currently {request.status} and may not accept new submissions.
          </AlertDescription>
        </Alert>
      )}
            ...
            {request.required_credentials.map((cred, index) => (
              <div key={index} className="p-4 border rounded-lg">
                <div className="flex items-start justify-between">
                  ...
                  <Badge variant={cred.essential ? "default" : "secondary"}>
                    {cred.essential ? "Required" : "Optional"}
                  </Badge>
                </div>
              </div>
            ))}
            ...
            <div className="grid gap-4 text-sm">
              ...
              <div className="space-y-2">
                ...
                <div className="pl-4 space-y-1 text-xs">
                  {request.valid_from && (
                    <div>Valid From: {formatDate(request.valid_from)}</div>
                  )}
                  {request.valid_until && (
                    <div>Valid Until: {formatDate(request.valid_until)}</div>
                  )}
                  {request.expires_at && (
              ...
              <div className="space-y-2">
                ...
                <div className="pl-4 space-y-1 text-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={request.verification_options.verify_signature ? "default" : "secondary"}>
                      {request.verification_options.verify_signature ? "Enabled" : "Disabled"}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Expiration Check:</span>
                    <Badge variant={request.verification_options.verify_expiration ? "default" : "secondary"}>
                      {request.verification_options.verify_expiration ? "Enabled" : "Disabled"}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Revocation Check:</span>
                    <Badge variant={request.verification_options.verify_revocation ? "default" : "secondary"}>
                      {request.verification_options.verify_revocation ? "Enabled" : "Disabled"}
                    </Badge>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {request.metadata && Object.keys(request.metadata).length > 0 && (
                      <div className="space-y-1">
                        ...
                        <pre className="pl-4 text-xs bg-muted p-2 rounded overflow-auto">
          ...
          {!canSubmit && (
            <p className="text-sm text-muted-foreground text-center mt-2">
              {isExpired 
                ...
                : "This request is not accepting submissions"
              }
            </p>
          )}
        </CardContent>
