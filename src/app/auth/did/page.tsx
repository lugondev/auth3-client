...
import {initiateDIDAuth, completeDIDAuth, validateSignature} from '@/services/didService'
	...
	const renderDIDInput = () => (
		<div className='space-y-4'>
			<div className='space-y-2'>
				...
				<Input id='did' type='text' placeholder='did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK' value={didInput} onChange={(e) => setDidInput(e.target.value)} disabled={loading} />
				<p className='text-sm text-muted-foreground'>Enter your DID to authenticate. Supported methods: did:key, did:web, did:ethr, did:ion, did:peer</p>
			</div>
	...
	const renderChallenge = () => (
		<div className='space-y-4'>
			<div className='space-y-2'>
				...
				<div className='p-3 bg-muted rounded-md font-mono text-sm break-all'>{challengeData?.challenge}</div>
				<p className='text-sm text-muted-foreground'>{challengeData?.instructions}</p>
			</div>

			<div className='flex gap-2'>
				<Button variant='outline' onClick={() => setShowQRCode(!showQRCode)} className='flex-1'>
					<QrCode className='mr-2 h-4 w-4' />
					{showQRCode ? 'Hide' : 'Show'} QR Code
				</Button>
				<Button variant='outline' onClick={() => setStep('signature')} className='flex-1'>
			...
			{showQRCode && (
				<div className='p-4 bg-white rounded-md border text-center'>
					<div className='text-sm text-muted-foreground mb-2'>Scan with your DID wallet</div>
					<div className='font-mono text-xs break-all p-2 bg-muted rounded'>{qrCodeData}</div>
				</div>
			)}

			<div className='text-xs text-muted-foreground'>Challenge expires at: {challengeData?.expires_at && new Date(challengeData.expires_at).toLocaleString()}</div>
		</div>
	)
	...
	const renderSignatureInput = () => (
		<div className='space-y-4'>
			<div className='space-y-2'>
				...
				<Input id='verification-method' type='text' placeholder='did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK#z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK' value={verificationMethod} onChange={(e) => setVerificationMethod(e.target.value)} disabled={loading} />
			</div>

			<div className='space-y-2'>
				<Label htmlFor='signature'>Signature</Label>
				<Input id='signature' type='text' placeholder='Enter the signature generated by your DID wallet' value={signature} onChange={(e) => setSignature(e.target.value)} disabled={loading} />
				<p className='text-sm text-muted-foreground'>Provide the signature generated by signing the challenge with your DID's private key</p>
			</div>

			<div className='flex gap-2'>
				<Button variant='outline' onClick={() => setStep('challenge')} disabled={loading} className='flex-1'>
					Back to Challenge
				</Button>
	...
	const renderComplete = () => (
		<div className='space-y-4 text-center'>
			...
			<div>
				...
				<p className='text-muted-foreground'>You have been successfully authenticated with your DID.</p>
			</div>
		...
		<div className='container mx-auto max-w-md py-8'>
			<Card>
				<CardHeader className='text-center'>
					...
					<CardDescription>Authenticate using your Decentralized Identifier</CardDescription>
				</CardHeader>
