...
export default function ProtectedLayout({children}: {children: React.ReactNode}) {
	...
	const isTenantSpace = /^\/dashboard\/tenant\/[^\/]+/.test(pathname) || 
	                     pathname.match(/^\/dashboard\/tenant\/[a-zA-Z0-9-_]+/)
	...
	if (pathname.split('/').length >= 4 && pathname.includes('/dashboard/tenant/')) {
		...
		return (
			<ProtectedWrapper key={`tenant-${pathname}`}>
				{children}
			</ProtectedWrapper>
		)
	}
	...
	if (isTenantSpace || isAdminSpace) {
		return (
			<ProtectedWrapper key={`no-shell-${pathname}`}>
				{children}
			</ProtectedWrapper>
		)
	}
