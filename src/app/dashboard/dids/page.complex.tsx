...
import {FileText, Settings, Search, Plus, Globe, AlertTriangle, Users, Key, Eye, Trash2, Power, MoreHorizontal, Coins, Network, User} from 'lucide-react'
...
const DIDManagementDashboard: React.FC<DIDManagementDashboardProps> = ({userId: propUserId, tenantId: propTenantId}) => {
	...
	// Load statistics using dedicated API endpoint
	const loadStats = useCallback(async () => {
		try {
			...
			const localStats: DIDStats = {
				...
				updated24h: statsResponse.dids_created_today || 0, // Use today's created DIDs
				methods: statsResponse.dids_by_method || {},
				byMethod: statsResponse.dids_by_method || {},
				userManaged: statsResponse.user_managed_dids || 0,
				systemManaged: statsResponse.system_managed_dids || 0,
			}
	...
	useEffect(() => {
		if (userId && !hasLoadedRef.current) {
			...
			// Load both DIDs and statistics concurrently
			Promise.all([
				fetchDIDs(1),
				loadStats()
			]).catch(err => {
				...
				<CardContent>
					{userDIDs.length === 0 ? (
						<div className='text-center py-8'>
							...
							<p className='text-sm text-muted-foreground mb-4'>Start by creating a new DID or registering an existing one you control</p>
							<div className='flex gap-2 justify-center'>
								<Link href='/dashboard/dids/register'>
		...
		<div className='space-y-4'>
			...
			{userDIDs.length === 0 ? (
				<div className='text-center py-12'>
					...
					<p className='text-gray-500 mb-4'>Get started by creating a new DID or registering an existing one</p>
					<div className='flex gap-2 justify-center'>
						<Link href='/dashboard/dids/register'>
							<Button variant='secondary'>
								<Key className='h-4 w-4 mr-2' />
